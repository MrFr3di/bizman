...
4. Before opening or updating a PR, run the canonical local gate: `uv run ruff check src`, `uv run lint-imports`, `uv run python -m compileall -q src tools tests`, `uv run --locked --with coverage==7.16.1 coverage run -m unittest discover -s tests -v`, `uv run --locked --with coverage==7.16.1 coverage xml`, and `uv run python tools/validate_repo.py`.
