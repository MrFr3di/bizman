...
class CorrelationTests(unittest.TestCase):
    ...
    def test_matching_submit_emits_strong_link(self):
        ...
        self.assertEqual(correlator.observe(action("a1", t=10.0)), [])
        links = correlator.observe(request("r1", t=10.1))
    ...
    def test_different_target_or_conflicting_frame_never_correlates(self):
        correlator = self._correlator()
        correlator.observe(action("a1", t=10.0, target="t1", frame="f1"))
        self.assertEqual(
            correlator.observe(request("r1", t=10.05, target="t2", frame="f1")),
            [],
        )
        self.assertEqual(
            correlator.observe(request("r2", t=10.05, target="t1", frame="f2")),
            [],
        )
    ...
    def test_stale_action_is_ignored(self):
        correlator = self._correlator()
        correlator.observe(action("a1", t=10.0))
        self.assertEqual(correlator.observe(request("r1", t=12.5)), [])

    def test_one_action_can_link_multiple_requests_but_request_only_once(self):
        correlator = self._correlator()
        correlator.observe(action("a1", t=30.0))
        first = correlator.observe(request("r1", t=30.05))
        second = correlator.observe(request("r2", t=30.08))
        duplicate = correlator.observe(request("r1", t=30.05))
    ...
    def test_later_action_can_match_recent_unlinked_request(self):
        correlator = self._correlator()
        self.assertEqual(correlator.observe(request("r1", t=40.1)), [])
        links = correlator.observe(action("a1", t=40.0))
        self.assertEqual(len(links), 1)
    ...
    def test_best_action_wins_for_one_request(self):
        correlator = self._correlator()
        correlator.observe(
            action("weak", t=50.0, kind="click", path=None, method=None)
        )
        correlator.observe(action("strong", t=50.09))
        links = correlator.observe(request("r1", t=50.1))
        self.assertEqual(len(links), 1)
